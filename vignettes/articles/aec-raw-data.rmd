...
The `National list of candidates` dataset contains a comprehensive list of all candidates who participated in Australian federal elections. It covers both `Senate` and `House` elections from `2004` to the present. The `2004` dataset is processed in the following way:
...
The `Seats decided on first preferences` dataset provides a summary of the seats that were decided on first preferences (i.e., the winning candidate in a division achieved equal to or more than 50% of the primary vote), detailing:
...
- The number of votes that the party received by vote type (`OrdinaryVotes`, `AbsentVotes`, `ProvisionalVotes`, `PrePollVotes`, `PostalVotes`, `TotalVotes`).
...
```{r, echo = FALSE}
df <- suppressMessages(
  get_aec_data(file_name = "First preferences by party")
)
...
- The division details (`StateAb`, `DivisionID`, `DivisionNm`).
- The cadndiate details (`CandidateID`, `Surname`, `GivenNm`), including their position on the ballot position (`BallotPosition`), and whether they were elected (`Elected`) or an incumbent (`HistoricElected`), and their the party (`PartyAb`, `PartyNm`) that they represent.
- The number of votes that the candidate received by vote type (`OrdinaryVotes`, `AbsentVotes`, `ProvisionalVotes`, `PrePollVotes`, `PostalVotes`, `TotalVotes`).
...
- The division details (`StateAb`, `DivisionID`, `DivisionNm`).
- The cadndiate details (`CandidateID`), including their ballot details (`BallotPosition`,`CandidateDetails`, `Group`), and whether they were elected (`Elected`) or an incumbent (`HistoricElected`), and their the party (`PartyAb`, `PartyNm`) that they represent.
- The number of votes that the candidate received by vote type (`OrdinaryVotes`, `AbsentVotes`, `ProvisionalVotes`, `PrePollVotes`, `PostalVotes`, `TotalVotes`).

Covering elections from `2004` to the present, this dataset is available for the `Senate` only. Minor amendments are made to the original dataset, changing `PartyName` to `PartyNm`. This ensures consistency in column names across all datasets.
...
```{r, echo = FALSE}
df <- suppressMessages(
  get_aec_data(file_name = "First preferences by candidate by polling place")
)
