...
The `National list of candidates` dataset contains a comprehensive list of all candidates who participated in Australian federal elections. It covers both `Senate` and `House` elections from `2004` to the present. When `process = TRUE`, the dataset is standardised: for the `2004` election, the `SittingMemberFl` column is removed and replaced with `Elected` (indicating winners, derived from the `Senators elected` and `Member elected` datasets) and `HistoricElected` (indicating incumbents, based on `SittingMemberFl`). This dataset enables you to:
...
df <- get_aec_data(
  ...
  date_range = list(from = "2011-01-01", to = "2023-01-01"),
  category = "House"
)
```
<br>
...
Covering elections from `2004` to the present, these datasets are available for both the `House` and `Senate` categories. In order to standardise datasets across election years, additional processing is available for election years prior to `2010`. This renames the pre-2010 columns from `Total` and `LastElectionTotal` to `National` and `LastElection`, respectively. It also removes `PartyAb`. No additional processing is required for the `Senate`, as the data is already standardised across all election years.
...
df <- get_aec_data(
  ...
  date_range = list(from = "2010-01-01", to = "2023-01-01"),
  category = "House"
)
```
<br>
...
#### First preferences by state by party
...
### `Senate`
#### First preferences by state by vote type
...
#### First preferences by group by vote type

<br>

#### First preferences by state by group by vote type

<br>

## House Two Party Preferred
...
df <- get_aec_data(
  ...
  date_range = list(from = "2016-01-01", to = "2023-01-01"),
  category = "House"
)
```
<br>
...
## House Distribution of Preferences
...
#### Two party preferred flow of preferences by state by party
...
#### Two candidate preferred flow of preferences by state by party
...
#### Two candidate preferred flow of preferences by polling place
